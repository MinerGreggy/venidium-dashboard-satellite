...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'venidium-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('venidium').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('venidium')[service]);
